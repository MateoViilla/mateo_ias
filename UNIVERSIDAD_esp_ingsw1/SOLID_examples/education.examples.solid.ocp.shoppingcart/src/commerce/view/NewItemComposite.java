...
public class NewItemComposite extends Composite {
	...
	public NewItemComposite(CommerceWindow window, Composite parent, int style) {
		...
		Button btnRemoveItem = new Button(grpNewItem, SWT.NONE);
		btnRemoveItem.setToolTipText("Remove the selected item from the cart");
		...
		FormData fd_btnRemoveItem = new FormData();
		fd_btnRemoveItem.top = new FormAttachment(btnAddToCart, 0, SWT.TOP);
		fd_btnRemoveItem.left = new FormAttachment(btnAddToCart, 6);
		btnRemoveItem.setLayoutData(fd_btnRemoveItem);
		btnRemoveItem.setText("Remove Item");
		
		Button btnBuyProducts = new Button(grpNewItem, SWT.NONE);
		btnBuyProducts.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				controller.buyProductsFromCart();
			}
		});
		btnBuyProducts.setToolTipText("Buy all the products in the cart");
		FormData fd_btnBuyProducts = new FormData();
		fd_btnBuyProducts.top = new FormAttachment(btnAddToCart, 0, SWT.TOP);
		fd_btnBuyProducts.left = new FormAttachment(btnRemoveItem, 6);
		btnBuyProducts.setLayoutData(fd_btnBuyProducts);
		btnBuyProducts.setText("Buy Products");
		
		Button btnSeeTotalMoney = new Button(grpNewItem, SWT.NONE);
		btnSeeTotalMoney.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				controller.viewCommerceTotalMoney();
			}
		});
		FormData fd_btnSeeTotalMoney = new FormData();
		fd_btnSeeTotalMoney.top = new FormAttachment(btnAddToCart, 0, SWT.TOP);
		fd_btnSeeTotalMoney.left = new FormAttachment(btnBuyProducts, 6);
		btnSeeTotalMoney.setLayoutData(fd_btnSeeTotalMoney);
		btnSeeTotalMoney.setText("See Total Money");
